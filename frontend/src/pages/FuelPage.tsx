      ...
      {fuelStats && (
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <p className="text-2xl font-bold text-gray-900 mt-1">${Number(fuelStats.summary.total_cost).toLocaleString()}</p>
          </div>
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            <p className="text-xs text-gray-500 uppercase tracking-wide">Avg Cost/Liter</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">${Number(fuelStats.summary.avg_cost_per_liter || 0).toFixed(2)}</p>
          </div>
        </div>
      )}
          ...
          <tbody className="divide-y divide-gray-100">
            {(records || []).map((r) => (
              <tr key={r.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">${Number(r.cost).toFixed(2)}</td>
