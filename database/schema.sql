-- Consolidated Database Schema for Transport and Fleet Management System
-- Run this once to set up or reset the database

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

DROP TABLE IF EXISTS maintenance_inventory_usage CASCADE;
DROP TABLE IF EXISTS activity_logs CASCADE;
DROP TABLE IF EXISTS sessions CASCADE;
DROP TABLE IF EXISTS password_reset_tokens CASCADE;
DROP TABLE IF EXISTS notifications CASCADE;
DROP TABLE IF EXISTS trip_attendance CASCADE;
...
CREATE INDEX idx_trips_available ON trips(status) WHERE status = 'available';
...
('users.roles', 'users', 'Manage user roles'),
...
('drivers.assign', 'drivers', 'Assign drivers to trips'),
...
('mechanics.assign', 'mechanics', 'Assign mechanics to tasks'),
...
('trips.assign', 'trips', 'Assign vehicles/drivers to trips'),
...
('maintenance.approve', 'maintenance', 'Approve maintenance work'),
...
('profile.upload', 'profile', 'Upload profile photo'),
('settings.view', 'settings', 'View system settings'),
('settings.edit', 'settings', 'Edit system settings'),
('dashboard.admin', 'dashboard', 'Access admin dashboard'),
('dashboard.driver', 'dashboard', 'Access driver dashboard'),
('dashboard.mechanic', 'dashboard', 'Access mechanic dashboard'),
('dashboard.client', 'dashboard', 'Access client dashboard')
ON CONFLICT (name) DO NOTHING;
...
-- Client permissions (both client and farm_client)
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permissions p
